...
SELECT 
    SUM(poster_qty) AS total_poster_sales
FROM
    orders;
...
-- so far try finding - what is the MEDIAN total_usd spent on all orders? Note, this is more advanced than the topics
-- we have covered thus far to build a general solution, but we can hard code a solution in the following way.
...
SELECT 
    a.name, SUM(total_amt_usd) total_sales
FROM
    orders o
        JOIN
...
SELECT 
    w.occurred_at, w.channel, a.name
FROM
    web_events w
        JOIN
...
-- What was the smallest order placed by each account in terms of total usd. Provide only two columns - the account name and
-- the total usd. Order from smallest dollar amounts to largest.
SELECT 
    a.name, MIN(total_amt_usd) smallest_order
FROM
    accounts a
        JOIN
    orders o ON a.id = o.account_id
...
SELECT 
    s.name, w.channel, COUNT(*) num_events
FROM
    accounts a
        JOIN
...
-- Order your table with the highest number of occurrences first.
SELECT 
    r.name, w.channel, COUNT(*) num_events
FROM
    accounts a
        JOIN
    web_events w ON a.id = w.account_id
...
-- only one region. If each account was associated with more than one region, the first query should have returned more rows
...
-- Actually all of the sales reps have worked on more than one account. The fewest number of accounts any sales rep works on
-- is 3. There are 50 sales reps, and they all have more than one account. Using DISTINCT in the second query assures that
-- all of the sales reps are accounted for in the first query.

SELECT 
    s.id, s.name, COUNT(*) num_accounts
FROM
    accounts a
        JOIN
    sales_reps s ON s.id = a.sales_rep_id
...
-- and technically, we can get this using a SUBQUERY as shown below. This same logic can be used for the other queries, 
...
SELECT 
    a.id, a.name, SUM(o.total_amt_usd) total_spent
FROM
    accounts a
        JOIN
    orders o ON a.id = o.account_id
...
SELECT 
    a.id, a.name, w.channel, COUNT(*) use_of_channel
FROM
    accounts a
        JOIN
    web_events w ON a.id = w.account_id
...
-- If we look further at the monthly data, we see that for 2013 and 2017 there is only one month of sales for each of these
-- years (12 for 2013 and 1 for 2017). Therefore, neither of these are evenly represented. Sales have been increasing year
-- over year, with 2016 being the largest sales to date. At this rate, we might expect 2017 to have the largest sales.
...
GROUP BY s.name
ORDER BY 2 DESC;
-- It is worth mentioning that this assumes each name is unique - which has been done a few times. We otherwise would want to break by
-- the name and the id of the table.
...
-- which are sales reps associated with more than 200 orders or more than 750000 in total sales. The middle group has any
-- rep with more than 150 orders or 500000 in sales. Create a table with the sales rep name, the total number of orders,--
-- total sales across all orders, and a column with top, middle, or low depending on this criteria. Place the top sales people
