...
-- sorted first by the account ID (in ascending order), and then by the total dollar
...
-- Compare the results of these two queries above. How are the results different when you switch the 
-- column you sort on first?
-- ANSWER : In query #1, all of the orders for each account ID are grouped together, and then within each of
--  those groupings, the orders appear from the greatest order amount to the least. 
...
-- Create a column that divides the standard_amt_usd by the standard_qty to find the unit price for standard paper for each 
...
SELECT 
    ...
    gloss_amt_usd,
    poster_amt_usd,
    (poster_amt_usd / (standard_amt_usd + gloss_amt_usd + poster_amt_usd)) * 100 AS Average
FROM
    orders
LIMIT 10;
...
-- So the answer to the question is that yes, the BETWEEN operator in SQL is inclusive; that is, the endpoint values are
-- included. So the BETWEEN statement in this query is equivalent to having written "WHERE gloss_qty >= 24 AND gloss_qty <= 29."



-- You will notice that using BETWEEN is tricky for dates! While BETWEEN is generally inclusive of endpoints, it assumes the time 
